...
src/history/                    session records in AsyncStorage, day grouping and totals, History screen
src/notifications/              goal-reached local notification (expo-notifications), scheduled on every transition
...
| Goal-reached notification | Scheduled at the goal instant while running, cancelled on pause/stop; banner shown even in the foreground | Unit tests; simulator: banner appeared at 1:01 elapsed of a 1m goal with the app backgrounded |
| Session history | Record appended on stop, capped at 500, grouped by day with totals | Unit tests; simulator |
